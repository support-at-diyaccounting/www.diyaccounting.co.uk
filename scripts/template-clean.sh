...
for file in $(find "$PROJECT_ROOT" -type f \( \
  ...
  -not -name "template-clean.sh" \
  -not -name "template-init.sh"); do
    sedi 's|@diy-accounting-uk/www-diyaccounting-co-uk|@owner/www-site-example|g' "$file"
    sedi 's|@diy-accounting-uk/www\.site\.example|@owner/www.site.example|g' "$file"
    sedi 's|diy-accounting-uk/www\.site\.example|owner/www.site.example|g' "$file"
    sedi 's|diy-accounting-uk|owner|g' "$file"
done
