...
import static co.uk.diyaccounting.gateway.utils.KindCdk.cfnOutput;
import static co.uk.diyaccounting.gateway.utils.KindCdk.ensureLogGroupWithDependency;
...
public class GatewayStack extends Stack {
    ...
    @Value.Immutable
    public interface GatewayStackProps extends StackProps {
        @Override
        Environment getEnv();
    ...
    public GatewayStack(final Construct scope, final String id, final GatewayStackProps props) {
        ...
        ResponseHeadersPolicy responseHeadersPolicy = ResponseHeadersPolicy.Builder.create(
                ...
                .comment("Security headers for gateway static site")
        ...
        // Generated by scripts/build-gateway-redirects.cjs from redirects.toml
        ...
        if (Files.exists(redirectFunctionPath)) {
            try {
                ...
                var redirectFunction = Function.Builder.create(this, resourcePrefix + "-RedirectFunction")
                        ...
                        .comment("301 redirects for old www.diyaccounting.co.uk URLs")
                        .build();
        ...
        infof("GatewayStack %s created for %s", this.getNode().getId(), String.join(", ", props.domainNames()));
    }
}
